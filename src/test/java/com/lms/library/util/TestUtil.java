...
/**
 * Utility class providing helper methods for unit and integration tests. This class includes
 * methods to reset static state in repository classes and to create preconfigured test entities
 * such as {@link Member}, {@link Loan}, {@link LoanItem}, and various {@link LibraryItem} types.
 *
 * @author Joel Silva
 * @version 1.0
 * @since 2025
...
public class TestUtil {
  ...
  /**
   * Resets the static state of {@link MemberRepository} by clearing its internal store and
   * resetting its ID sequence to {@code 1}.
   *
  ...
  /**
   * Resets the static state of {@link LoanRepository} by clearing its internal store and resetting
   * its ID sequence to {@code 1}.
   *
   * @throws Exception if reflection access to repository fields fails
   */
  ...
  /**
   * Resets the static state of {@link LoanItemRepository} by clearing its internal store and
   * resetting its ID sequence to {@code 1}.
   *
   * @throws Exception if reflection access to repository fields fails
   */
  ...
  /**
   * Resets the static state of {@link LibraryItemRepository} by clearing its internal store and
   * resetting its ID sequence to {@code 1}.
   *
   * @throws Exception if reflection access to repository fields fails
   */
  ...
  public static Loan createTestLoan(
      Long id, Member member, LocalDate loanDate, LocalDate dueDate, LibraryItem item) {
  ...
  /**
   * Creates a test {@link Loan} with a generated {@link Member}, a default date range, and a single
   * test {@link Book} item.
   *
   * @param id the ID to assign to the loan
   * @return a new {@link Loan} instance populated with test data
  ...
  /**
   * Creates a {@link LoanItem} for the given loan and library item, with the option to mark it as
   * returned.
   *
   * @param loan the associated {@link Loan}
   * @param item the {@link LibraryItem} being loaned
  ...
  public static LibraryItem createTestJournal(Long id) {
    Journal journal =
        new Journal(
            "Test Journal", "Test Publisher", LocalDate.now(), "11112222", "Test Publisher", 1, 1);
    journal.setId(id);
    return journal;
  }
}
