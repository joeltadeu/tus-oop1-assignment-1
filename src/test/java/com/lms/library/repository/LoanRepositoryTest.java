...
class LoanRepositoryTest {
    ...
    void save_NewLoanWithoutId_ShouldGenerateId() {
        // Arrange
        var loan = createTestLoan(null, testMember1,
                LocalDate.now(), LocalDate.now().plusWeeks(2), testBook);
    ...
    void save_MultipleLoans_ShouldGenerateSequentialIds() {
        // Arrange
        var loan1 = createTestLoan(null, testMember1, LocalDate.now(), LocalDate.now().plusWeeks(2), testBook);
        var loan2 = createTestLoan(null, testMember2, LocalDate.now(), LocalDate.now().plusWeeks(2), testBook);
        var loan3 = createTestLoan(null, testMember1, LocalDate.now(), LocalDate.now().plusWeeks(1), testBook);
    ...
    void findById_ExistingId_ShouldReturnLoan() {
        // Arrange
        var loan = createTestLoan(null, testMember1, LocalDate.now(), LocalDate.now().plusWeeks(2), testBook);
        var savedLoan = repository.save(loan);
        var loanId = savedLoan.getId();
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            threads[i] = new Thread(() -> {
                for (int j = 0; j < loansPerThread; j++) {
                    ...
                    var loan = createTestLoan(null, member,
                            LocalDate.now().plusDays(j),
                            LocalDate.now().plusWeeks(2), testBook);
    ...
    void mixedOperations_ShouldWorkCorrectly() {
        ...
        var today = LocalDate.now();
        var loan1 = repository.save(createTestLoan(null, testMember1, today.minusDays(10), today.plusWeeks(1), testBook));
        repository.save(createTestLoan(null, testMember2, today.minusDays(5), today.plusWeeks(2), testBook));
        var loan3 = repository.save(createTestLoan(null, testMember1, today.minusDays(2), today.plusWeeks(3), testBook));
        var loan4 = repository.save(createTestLoan(null, testMember1, today, today.plusWeeks(4), testBook));
    ...
    void save_LoanWithPredefinedId_ShouldUseProvidedId() {
        // Arrange
        var loan = createTestLoan(100L, testMember1, LocalDate.now(), LocalDate.now().plusWeeks(2), testBook);
